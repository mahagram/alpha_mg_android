...
public class WalletRequestCardAdapter extends RecyclerView.Adapter<WalletRequestCardAdapter.ViewHolder> {
    ...
    public WalletRequestCardAdapter(Context context, List<WalletRequestReportItems> AEPSRequestItems)
    {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
        WalletRequestReportItems items= AEPSRequestItems.get(i);
        ...
        viewHolder.textview_remark.setText("Remarke : "+items.getRemark());
        ...
        if (i==AEPSRequestItems.size()-1) {
            if (WalletRequestsReports.last_array_empty) {

            } else {
                ((WalletRequestsReports)context).mCallNextList();
            }
        }
